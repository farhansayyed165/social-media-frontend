...
const Carousel = forwardRef(function ({ items, id, maxW, edit, handleEditClick }, ref) {
    ...
    const slides = items.map((item, i) => {
        return (
            <div className={`${slideState == i ? "current-slide" : "hidden"} w-full bg-gray-30 transition-all duration-700 ease-in-out flex justify-center items-center object-center z-40`} data-carousel-item key={i} slide={i}>
                ...
                {
                    edit ? <span className='absolute top-0 left-0 z-[1000] pt-2 pl-2 ' slide={i} remove={false} onClick={handleEditClick}><ImCross size={20} slide={i} style={{ stroke: "black", strokeWidth: "1x"}} color='white' /> <span slide={i}></span></span> : <></>
                }
